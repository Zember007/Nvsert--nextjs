...
export default function useHeadLayout(configs, fileConfigs) {
  ...
  useEffect(() => {
    ...
    if (configs.COUNTERS_HEAD) {
      ...
      // Обрабатываем все теги (link, meta, script, noscript, style)
      ['link', 'meta', 'script', 'noscript', 'style'].forEach((tag) => {
        Array.from(headCounters.getElementsByTagName(tag)).forEach((element) => {
          ...
          // Убедимся, что каждый массив существует перед добавлением элемента
          if (!parsedMeta[tag]) {
            parsedMeta[tag] = [];
          }
    ...
    // Обновляем состояние с метаданными
