...
const LinkButtonTitle = ({ title, link }: { title: string, link: string }) => {
    ...
    return (
        <Link prefetch={false} href={localizePath(link)} className={`flex items-center gap-[10px] group ${stylesBtn.lineAfterBox}`}>
            <span className={`text-[16px] !font-normal ${stylesBtn.lineAfter} group-active:scale-[.95] whitespace-nowrap transition-all duration-100`}>{title}</span>
