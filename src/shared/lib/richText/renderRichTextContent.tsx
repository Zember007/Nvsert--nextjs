...
/** Parses inline markup in a string: **bold**, _italic_, <u>underline</u>, ~~strike~~, [text](url) */
function parseInlineRichText(text: string): React.ReactNode[] {
  ...
  while (remaining.length > 0) {
    ...
    if (earliest.type === 'bold') {
      nodes.push(React.createElement('strong', { key: `inline-${key++}` }, ...parseInlineRichText(content)));
    } else if (earliest.type === 'italic') {
      nodes.push(React.createElement('em', { key: `inline-${key++}` }, ...parseInlineRichText(content)));
    } else if (earliest.type === 'underline') {
      nodes.push(React.createElement('u', { key: `inline-${key++}` }, ...parseInlineRichText(content)));
...
export function renderRichTextContent(text: string, small?: boolean): React.ReactNode[] {
  text = text.trim();
  ...
  const flushList = (forceType?: 'ordered' | 'unordered') => {
    if (currentListItems.length > 0) {
      ...
      const liClassName = `font-light ${small ? `${textSize.text2}` : `${textSize.textBasePost}`}`;
  ...
  lines.forEach((line, index) => {
    ...
    elements.push(
      <p
        key={`p-${index}`}
        className={`-my-[5px] ${small ? `${textSize.text2}` : `${textSize.textBasePost}`}`}
      >
