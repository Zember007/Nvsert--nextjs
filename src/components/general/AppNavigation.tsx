    ...
    return (
        <>
            <div ref={setWrapperRef} className={`tariff-wrap ${className} !max-w-[250px] `}>
                <Link ref={setButtonRef} href={'/' + link} className={`tariff text-left h-[72px] no-transitions overflow-hidden not-backdrop flex  xl:gap-[10px] gap-[5px] group/img p-[5px] rounded-[3px] items-center    border-solid  border border-[transparent] ${dark ? 'hover:border-[#34446D] hover:bg-[#F5F5F2]' : 'hover:border-[#fff] hover:bg-[#34446d33]'}`}>
                    <motion.div
                    ...
                    <p className={`${dark ? 'text-[#000] text-[18px]' : 'text-[#FFF] xl:text-[18px] text-[14px]'} leading-[1.1] pr-[6px] whitespace-pre-line max-w-full`}>{filterPrepositions(title)}</p>
                </Link>
            </div>
        </>
    )
