    ...
    return (
        <>
            <div ref={setWrapperRef} className={`tariff-wrap ${className} !max-w-[250px] `}>
                <Link ref={setButtonRef} href={'/' + link} className={`tariff text-left  no-transitions overflow-hidden not-backdrop flex  gap-[10px] group/img p-[5px] rounded-[3px] items-center    border-solid  border border-[transparent] ${dark ? 'hover:border-[#34446D] hover:bg-[#F5F5F2] h-[72px]' : 'xxxxl:h-[72px] h-[60px] hover:border-[#fff] hover:bg-[#34446d33]'}`}>
                    <motion.div
                    ...
                    <p className={`${dark ? 'text-[#000] text-[18px]' : 'text-[#FFF] xxxxl:text-[18px] xl:text-[16px] text-[14px]'} leading-[1.1] pr-[6px] whitespace-pre-line max-w-full`}>{filterPrepositions(title)}</p>
                </Link>
            </div>
        </>
    )
