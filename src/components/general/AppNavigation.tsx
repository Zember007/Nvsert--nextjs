    ...
    return (
        <>
            <div ref={setWrapperRef} className={`tariff-wrap ${className} !max-w-[250px]`}>
                <Link ref={setButtonRef} href={'/' + link} className={`tariff text-left h-[72px] no-transitions overflow-hidden not-backdrop flex  xxxl:gap-[10px] gap-[5px] group/img pr-[6px] rounded-[4px] items-center    border-solid  border border-[transparent] ${dark ? 'hover:border-[#34446D] hover:bg-[#F5F5F2]' : 'hover:border-[#fff] hover:bg-[#34446d33]'}`}>
                    <motion.div
                    ...
                    <p className={`${dark ? 'text-[#000]' : 'text-[#FFF]'} whitespace-pre-line max-w-full  xl:text-[18px] text-[14px]`}>{filterPrepositions(title)}</p>
                </Link>
            </div>
        </>
    )
