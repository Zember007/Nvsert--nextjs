...
const QuestionsBlock = ({ setActive, active, number, title, text }: { setActive: (value: boolean) => void, active: boolean, number: number, title: string, text: string }) => {
    ...
    return (
        <div className={`relative flex flex-col rounded-[6px] ${active ? 'bg-[#34446D33]' : 'bg-[#FFFFFF26]'}  backdrop-blur-[1px] z-[0]`}>
            ...
            <div className={`max-h-[0px] transition-all easy-in duration-200 overflow-hidden ${active && '!max-h-[1000px]'}`}>
                <div className="pl-[110px] pr-[34px] py-[30px] flex flex-col gap-[30px]">
                    ...
                    <div
                        ...
                        <button
                            ref={setButtonRef} className='btnIconAn an-border border-[#34446D] border border-solid tariff text-[20px] transition-all duration-300 font-bold tracking-normal  gap-[6px]  text-[#FFF] rounded-[4px] group bg-[#34446D]  leading-[1]'>
