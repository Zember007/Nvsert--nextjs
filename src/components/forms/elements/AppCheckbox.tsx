...
const CustomCheckbox: React.FC<CustomCheckboxProps> = ({
    ...
    return (
        <div className="flex items-center s:gap-[6px] gap-[4px] no-drag"
            ...
            <span className={`leading-[0.8] transition-all duration-300 peer-checked:text-[#000] s:text-[16px] text-[14px] ${whiteBox ? 'text-[#FFF]' : 'text-[#000]'} ${fail && '!text-[#FF3030]'}`}>{label}</span>
        </div>
    );
};
