...
const AppMainForm = ({ btnText, bg = true, BounceWrapper, active, countTrigger }: { btnText: string; bg?: boolean; close?: () => void; BounceWrapper?: () => void; active?: boolean; countTrigger?: number }) => {
    ...
    return (
        <motion.div
            animate={controls}
            initial={{ x: 0 }}
            className={`relative main-form border border-solid ${bg ? 'bg-[#93969d26] border-[#93969d]' : 'border-[transparent]'} p-[20px] s:p-[40px] max-w-[320px] s:max-w-[400px] flex flex-col gap-[20px]  rounded-[6px]`}>
            {successMessageVisible && <FlightSuccess closeIcon={bg} text="Спасибо за заявку" close={() => { setSuccessMessageVisible(false) }} />}


            <span className={`leading-[0.7] text-[#000] text-[24px] s:text-[32px] font-light ${bg ? 'text-center' : 'text-left'} tracking-[-0.03em] s:px-[8px]  ${successMessageVisible && 'opacity-0'} `}>Оформить заявку</span>
